...
TEST(ImageTest, Constructor_RawData_InitializesProperly)
{
    ...
    Image img(Path(std::string("test/path")), "myimg", Image::eFormat::RGB8, 1, 1, data);
...
TEST(ImageTest, MoveConstructor_ValidImage_TransfersOwnership)
{
    ...
    Image src(Path(std::string("test/path")), "myimg", Image::eFormat::RGB8, 1, 1, data);
...
TEST(ImageTest, MoveAssignment_ValidImage_TransfersOwnership)
{
    ...
    Image src(Path(std::string("test/path")), "src", Image::eFormat::RGB8, 1, 1, data);

    uint8_t empty[3] = {0, 0, 0};
    Image dst(Path(std::string("old")), "old", Image::eFormat::RGB8, 1, 1, empty);
...
TEST(ImageTest, GetColor32_AllFormats_ReturnsCorrectColor)
{
    {
        uint8_t data[] = {128};
        Image img(Path(std::string()), "", Image::eFormat::R8, 1, 1, data);
    ...
    {
        uint8_t data[] = {10, 20};
        Image img(Path(std::string()), "", Image::eFormat::RG8, 1, 1, data);
        Color32 c = img.GetColor32(0, 0);
...

    {
        ...
        Image img(Path(std::string()), "", Image::eFormat::RGB8, 1, 1, data);
        Color32 c = img.GetColor32(0, 0);
...

    {
        ...
        Image img(Path(std::string()), "", Image::eFormat::RGBA8, 1, 1, data);
        Color32 c = img.GetColor32(0, 0);
...

    {
        ...
        Image img(Path(std::string()), "", Image::eFormat::RGBA32F, 1, 1, reinterpret_cast<uint8_t*>(data));
        Color32 c = img.GetColor32(0, 0);
        EXPECT_EQ(c.R, 255);
...
TEST(ImageTest, GetColor128_AllFormats_ReturnsCorrectColor)
{
    {
        float data[] = {0.75f};
        Image img(Path(std::string()), "", Image::eFormat::R32F, 1, 1, reinterpret_cast<uint8_t*>(data));
...

    {
        ...
        Image img(Path(std::string()), "", Image::eFormat::RG32F, 1, 1, reinterpret_cast<uint8_t*>(data));
        Color128 c = img.GetColor128(0, 0);
