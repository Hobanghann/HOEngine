...
{
    auto upImg = ResourceImporter::ImportImage(Path(std::string("this_file_does_not_exist_12345.png")));
    EXPECT_EQ(upImg, nullptr);
}
...
TEST(ResourceImporterTest, ImportImage_SingleChannelPNG_MapsToR8Format)
{
    ...
    auto upImg = ResourceImporter::ImportImage(Path(pathStr));

    ASSERT_NE(upImg, nullptr);
    EXPECT_EQ(upImg->GetWidth(), width);
    EXPECT_EQ(upImg->GetHeight(), height);
    EXPECT_EQ(upImg->GetFormat(), Image::eFormat::R8);
    EXPECT_EQ(upImg->GetColor32(0, 0).R, 10);
    EXPECT_EQ(upImg->GetColor32(0, 1).R, 30);
}
...
TEST(ResourceImporterTest, ImportImage_DualChannelPNG_MapsToRG8Format)
{
    ...
    ASSERT_NE(upImg, nullptr);
    EXPECT_EQ(upImg->GetFormat(), Image::eFormat::RG8);

    Color32 c0 = upImg->GetColor32(0, 0);
    EXPECT_EQ(c0.R, 10);
    EXPECT_EQ(c0.G, 50);

    Color32 c1 = upImg->GetColor32(1, 0);
    EXPECT_EQ(c1.R, 20);
    EXPECT_EQ(c1.G, 60);
}
...
TEST(ResourceImporterTest, ImportImage_TripleChannelPNG_MapsToRGB8Format)
{
    ...
    ASSERT_NE(upImg, nullptr);
    EXPECT_EQ(upImg->GetFormat(), Image::eFormat::RGB8);

    EXPECT_EQ(upImg->GetColor32(0, 0).R, 1);
    EXPECT_EQ(upImg->GetColor32(0, 0).G, 2);
    EXPECT_EQ(upImg->GetColor32(0, 0).B, 3);

    EXPECT_EQ(upImg->GetColor32(0, 1).R, 4);
    EXPECT_EQ(upImg->GetColor32(0, 1).G, 5);
    EXPECT_EQ(upImg->GetColor32(0, 1).B, 6);
}
...
TEST(ResourceImporterTest, ImportImage_QuadChannelPNG_MapsToRGBA8Format)
{
    ...
    ASSERT_NE(upImg, nullptr);
    EXPECT_EQ(upImg->GetFormat(), Image::eFormat::RGBA8);
...
TEST(ResourceImporterTest, ImportImage_HDR_MapsToRGB32FFormat)
{
    ...
    ASSERT_NE(upImg, nullptr);
    EXPECT_EQ(upImg->GetFormat(), Image::eFormat::RGB32F);

    Color128 c = upImg->GetColor128(0, 0);
...
static void runImporterTest(const Path& filePath)
{
    ...
    std::unique_ptr<ModelImportContext> upImportContext = ResourceImporter::ImportModel(pathStr, false, false);

    ASSERT_NE(upImportContext, nullptr) << "Importing model failed to load: " << pathStr.ToString();

    ASSERT_NE(upImportContext->pAssimpScene, nullptr) << "Assimp failed to load: " << pathStr.ToString();

    ASSERT_NE(upImportContext->pAssimpScene->mRootNode, nullptr)
        << "Root node is null for file: " << pathStr.ToString();

    ASSERT_FALSE(upImportContext->pFlattedScene.empty())
        << "topologicalSortRecursive produced empty result: " << pathStr.ToString();

    const aiNode* pExpectedRootNode = upImportContext->pAssimpScene->mRootNode;
    const aiNode* pFrontNode = upImportContext->pFlattedScene.front();
    EXPECT_EQ(pExpectedRootNode, pFrontNode) << "Root node is not the first element in FlattedScene.";

    EXPECT_EQ(upImportContext->pFlattedScene.size(), upImportContext->NodeToIndex.size())
        << "NodeToIndex size mismatch";

    for (int32_t i = 0; i < static_cast<int32_t>(upImportContext->pFlattedScene.size()); ++i)
    {
        aiNode* n = upImportContext->pFlattedScene[i];
        auto it = upImportContext->NodeToIndex.find(n);
        ASSERT_TRUE(it != upImportContext->NodeToIndex.end()) << "Node missing in NodeToIndex map";

        EXPECT_EQ(it->second, i) << "NodeToIndex index does not match flattened order";
    }

    EXPECT_GT(upImportContext->pAssimpScene->mNumMeshes, 0) << "Scene contains no meshes";

    const aiMesh* pAssimpMesh = upImportContext->pAssimpScene->mMeshes[0];
    ...
    {
        EXPECT_GE(upImportContext->pAssimpScene->mRootNode->mNumChildren, 0u) << "Expected child nodes in GLTF";
    }
}
